...
import { deletePermissions, getPermissionByMenu, insertPermissions, updatePermissions } from "@/repositories/permission.server";
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const menu = await ServiceFactory.getOne('menu', id)
    
    if (!menu) {
      return NextResponse.json({ message: "Menu not found" }, { status: 404 })
    }
    
    const existingData = await getPermissionByMenu(id)

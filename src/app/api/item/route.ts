  ...
  try {
    ...
    if (company.item_auto) {
      data = await prisma.$transaction(async tx => {
        ...
        company.item_format?.split('-').forEach((part, i) => {
        ...
        let seq = await ServiceFactory.getSequence('item', tx)
        if (!seq) {
...
export async function GET(req: NextRequest) {
  try {
    const { data, total } = await ServiceFactory.getAll(
      'item',
      req.nextUrl.searchParams,
      { uom: true, category: true }
    )
